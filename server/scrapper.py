...
def generate_page_report(url: str, content: bytes, company_name: str):
    ...
    description = description_meta["content"].strip() if description_meta and description_meta.get("content") else "No Description Found"
...
"""
    ...
    domain_name = url_domain.split('.')[-2] if len(url_domain.split('.')) > 2 else url_domain.split('.')[0]
...
def scrape_entire_website(start_url: str, company_name: str, max_iterations=1000):
    ...
    while urls_to_scrape:
        ...
        for a in soup.find_all("a", href=True):
            ...
            if parsed_joined_url.netloc == base_domain and joined_url not in scraped_urls and joined_url not in urls_to_scrape:
